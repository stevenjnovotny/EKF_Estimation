...
//  ekfState(6) = ekfState(6) + dtIMU * gyro.z;	// yaw
  ...
  // Try option 1 from above:
  float roll = rollEst; // phi
  float pitch = pitchEst; // theta
//  float yaw = 0;  // psi
...
//    double t3 = tan(roll);
    ...
    transformation matrix for converting body-frame angular rates to Euler angular rates
     */
    R(0,0) = 1;
    R(0,1) = s3 * t2;
    R(0,2) = c3 * t2;
    ...
    float predictedPitch = pitchEst + dtIMU * rates.y;
    float predictedRoll = rollEst + dtIMU * rates.x;
    ekfState(6) = ekfState(6) + dtIMU * rates.z;    // yaw
...
VectorXf QuadEstimatorEKF::PredictState(VectorXf curState, float dt, V3F accel, V3F gyro)
{
  ...
  ////////////////////////////// BEGIN STUDENT CODE ///////////////////////////
    ...
    V3F accel_r = attitude.Rotate_BtoI(accel);
    ...
    predictedState(3) = curState(3) + accel_r.x * dt; // vx
    predictedState(4) = curState(4) + accel_r.y * dt; // vy
    predictedState(5) = curState(5) - CONST_GRAVITY * dt + accel_r.z * dt ; // vz
...
MatrixXf QuadEstimatorEKF::GetRbgPrime(float roll, float pitch, float yaw)
{
  ...
  ////////////////////////////// BEGIN STUDENT CODE ///////////////////////////
    ...
    double c1 = cos(yaw);       // psi
    double c2 = cos(pitch);     // theta
    double c3 = cos(roll);      // phi
    double s1 = sin(yaw);
    double s2 = sin(pitch);
    double s3 = sin(roll);

    // from Estimation for Quadrotors by Tellex et al
    RbgPrime(0,0) = -c2 * s1;
    RbgPrime(0,1) = -s3 * s2 * s1 - c3 * c1;
    RbgPrime(0,2) = -c3 * s2 * s1 + s3 * c1;
    RbgPrime(1,0) = c2 * c1;
    RbgPrime(1,1) = s3 * s2 * c1 - c3 * s1;
    RbgPrime(1,2) = c3 * s2 * c1 + s3 * s1;
...
void QuadEstimatorEKF::Predict(float dt, V3F accel, V3F gyro)
{
  ...
  ////////////////////////////// BEGIN STUDENT CODE ///////////////////////////
    ...
    // using Mat3x3F function:  V3F operator*(const V3F& a) const
    // using V3F function: inline float sum() const{ return x+y+z;};
    gPrime(3,6) = (gPrime(0) * accel).sum() * dt;
    gPrime(4,6) = (gPrime(1) * accel).sum() * dt;
    gPrime(5,6) = (gPrime(2) * accel).sum() * dt;
...
void QuadEstimatorEKF::UpdateFromGPS(V3F pos, V3F vel)
{
  ...
  ////////////////////////////// BEGIN STUDENT CODE ///////////////////////////
    ...
    // -- I think this is the point of the whole project...
    zFromX(0) = ekfState(0);
    zFromX(1) = ekfState(1);
    zFromX(2) = ekfState(2);
    zFromX(3) = ekfState(3);
